...
export default class Registry {
	...
	set(key: string, item: any, ...rest: any[]) {
		if (isNil(key)) {
			throw new Error(
				`No key provided in the add method of ${this.name} registry.`
			);
		}

		if (isNil(item)) {
			throw new Error(
				`No item provided in the add method of ${this.name} registry.`
			);
		}
	...
	get(key: string): any {
		if (isNil(key)) {
			throw new Error(
				`No key provided in the get method of ${this.name} registry.`
			);
		}
	...
	has(key: string): boolean {
		if (isNil(key)) {
			throw new Error(
				`No key provided in the has method of ${this.name} registry.`
			);
		}
	...
	remove(key: string, ...rest: any[]) {
		if (isNil(key)) {
			throw new Error(
				`key cannot be ${key} in the remove method of ${this.name} registry.`
			);
		}
		if (!this.data.has(key)) {
