...
import { BlueBaseModuleRegistry } from '../BlueBaseModuleRegistry';
...
describe('BlueBaseModuleRegistry', () => {
	...
	describe('.set method', () => {
		...
		it('should set item', async () => {
			...
			const registry = new BlueBaseModuleRegistry<string, {}>(BB);

			registry.set('foo', { value: { default: 'bar' } });
			const result = await registry.resolve('foo');
		...
		// 	expect(() => registry.set('foo', { default: { value: 'bar' } })).toThrow();
		// });
