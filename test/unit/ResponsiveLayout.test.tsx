...
import ResponsiveLayout from '../../src/plugins/ResponsiveComponentsPlugin/components/ResponsiveLayout/index';
import BR from '../../src/index';
import { setWindowDimentions } from '../../src/plugins/ResponsiveComponentsPlugin/redux/actions';
...
describe('Responsive Layout tests', () => {
	it('should render the xs layout when size xs is passsed', () => {
		...
		expect(
			TestUtils.scryRenderedComponentsWithType(wrapper, xs).length
		).toEqual(1);
		expect(
			TestUtils.scryRenderedComponentsWithType(wrapper, defaultComponent).length
		).toEqual(0);
	});
	it('should render the sm layout when size sm is passsed', () => {
		...
		const wrapper = TestUtils.renderIntoDocument(
	 	...
	 	expect(
	 		TestUtils.scryRenderedComponentsWithType(wrapper, sm).length
		).toEqual(1);
	 	expect(
		TestUtils.scryRenderedComponentsWithType(wrapper, defaultComponent).length
	 	).toEqual(0);
	...
	it('should render the md layout when size md is passsed', () => {
		...
		expect(
			TestUtils.scryRenderedComponentsWithType(wrapper, md).length
		).toEqual(1);
		expect(
			TestUtils.scryRenderedComponentsWithType(wrapper, defaultComponent).length
		).toEqual(0);
	...
	it('should render the lg layout when size lg is passsed', () => {
		...
		expect(
			TestUtils.scryRenderedComponentsWithType(wrapper, lg).length
		).toEqual(1);
		expect(
			TestUtils.scryRenderedComponentsWithType(wrapper, defaultComponent).length
		).toEqual(0);
	...
	it('should render the xl layout when size xl is passsed', () => {
		...
		expect(
			TestUtils.scryRenderedComponentsWithType(wrapper, xl).length
		).toEqual(1);
		expect(
			TestUtils.scryRenderedComponentsWithType(wrapper, defaultComponent).length
		).toEqual(0);
	...
	describe('default layout when any of following layouts are not provided', () => {
		it('should render the default layout when size xs is not passsed ', () => {
			...
			expect(
				TestUtils.scryRenderedComponentsWithType(wrapper, defaultComponent)
					.length
			).toEqual(1);
		});
	...
	// 		expect(
	// 			TestUtils.scryRenderedComponentsWithType(wrapper, Heading).length
	// 		).toEqual(1);
	// 	});
	...
	// 		expect(
	// 			TestUtils.scryRenderedComponentsWithType(wrapper, Paragraph).length
	// 		).toEqual(1);
	// 	});
	...
	// 				<Provider store={mockStoreInitialized}>
	// 					<ResponsiveLayout default={defaultComponent} lg="NotRegistered" />
	// 				</Provider>
	// 			)
	...
	// 		const wrapper = TestUtils.renderIntoDocument(
	// 			<Provider store={mockStoreInitialized}>
	// 				<ResponsiveLayout default="" xs={xs} lg={lg} />
	// 			</Provider>
	// 		);
