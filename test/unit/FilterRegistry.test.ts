...
describe('Filters test specifications', () => {
	...
	describe('remove filter', () => {
		...
		it('should throw error because hook is null', () => {
			expect(() => BR.Filters.remove(null, 'func1')).toThrow('Hook cannot be null');
		});
		it('should throw error because hook is undefined', () => {
			expect(() => BR.Filters.remove(undefined, 'func1')).toThrow('Hook cannot be undefined');
		});
		it('should throw error because function is null', () => {
			expect(() => BR.Filters.remove('test.hook', null)).toThrow('Filter name cannot be null');
		});
